  ...
  module Core
    ...
    module ClassMethods
      ...
      def tagged_with(tags, options = {})
        ...
        request = select(select_clause).
          ...
          group(group).
          having(having).
          # esti@efaber.net: Changed order by reorder
    ...
    def tag_list_cache_on(context, locale=I18n.locale)
      ...
      if instance_variable_get(variable_name)
        ...
        instance_variable_set(variable_name, ActsAsTaggableOn::TagList.new(tags_on(context).map(&"name_#{locale}".to_sym)))
      end
    end
    ...
    def save_tags
      tagging_contexts.each do |context|
        ...
        ########## Modified by <tania@efaber.net> to support added_tags and removed_tags attributes ##########
        # if old_tags.present?
        #   old_taggings = taggings.where(:tagger_type => nil, :tagger_id => nil, :context => context.pluralize.to_s, :tag_id => old_tags)
        # end
        if old_tags.present?
          self.removed_tags.concat(old_tags.map(&:name))
          old_taggings = taggings.where(:tagger_type => nil, :tagger_id => nil, :context => context.pluralize.to_s, :tag_id => old_tags)
        end
        ########## End of Modified by <tania@efaber.net> to support added_tags and removed_tags attributes ##########
        ...
        #   taggings.create!(:tag_id => tag.id, :context => context.pluralize.to_s, :taggable => self)
        # end
        if new_tags.present?
          self.added_tags.concat(new_tags.map(&:name))
          new_tags.each do |tag|
            taggings.create!(:tag_id => tag.id, :context => context.pluralize.to_s, :taggable => self)
          end
        end
    ...
    ########## Modified by <tania@efaber.net> to support add_tags and remove_tags callbacks ##########
    def added_tags
      @added_tags ||= []
    end
    ...
    tag_types.each do |tag_type|
      context = tag_type.to_s.singularize

      # ['es', 'eu', 'en'].each do |lang|
      #   define_method "#{context}_list_#{lang}" do
      #     self.tag_list
      #     # return tag_list_on(context, lang)
      #   end
        
      #   define_method "#{context}_list_#{lang}=" do |new_tags|
      #     set_tag_list_on(context, new_tags)
      #   end

      #   define_method "#{context}_sanitized_name_list_#{lang}" do
      #     return send("#{context}_sanitized_name_list", lang)
      #   end
      # end

      define_method "#{context}_sanitized_name_list" do |lang = I18n.locale|
        variable_name = "@#{context}_sanitized_name_list"
        if instance_variable_get(variable_name)
          instance_variable_get(variable_name)
        else
          instance_variable_set(variable_name, ActsAsTaggableOn::TagList.new(*tags.map(&"sanitized_name_#{lang}".to_sym)))
        end
      end

    end
    ########## End of Modified by <tania@efaber.net> to support add_tags and remove_tags callbacks ##########
